...
<div class="bg-white p-6 rounded-lg shadow-md mt-8">
    ...
    <div class="mt-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
            @foreach ($diamondVariants as $index => $variant)
                @php $variationId = 'diamond_' . $variant['total']; @endphp
                <div class="variant-card flex items-center gap-4 bg-white p-4 rounded-xl shadow-sm border hover:border-blue-500 cursor-pointer transition"
                    data-variant="diamond{{ $variant['total'] }}"
                    data-price="{{ $variant['price'] }}"
                    data-variation-id="{{ $variationId }}">
                    
                    <img src="{{ asset('storage/image/icon-product/Clear-Diamonds.png') }}" alt="Diamond Icon"
                        class="w-12 h-12 object-contain">

                    <div class="flex flex-col text-sm">
                        <span class="font-semibold text-gray-800 flex items-center gap-1">
                            {{ $variant['nominal'] }} Diamonds
                        </span>
                        @if ($variant['bonus'] > 0)
                        <span class="text-green-600 font-normal flex items-center gap-1">
                            (+ Bonus {{ $variant['bonus'] }})
                        </span>
                        @endif
                        <span class="text-blue-600 font-medium">Rp {{ number_format($variant['price'], 0, ',', '.') }}</span>
                    </div>
                </div>
            @endforeach
        </div>
...
<!-- Payment Summary -->
    ...
    <button id="openPopup" 
            type="button" 
            class="bg-red-500 text-white w-full py-3 rounded-lg text-lg font-semibold mt-8">
            Beli
    </button>    
</div>
    <!-- Pop-up -->
    <div id="popupContainer" class="fixed inset-0 bg-black bg-opacity-50 items-center justify-center hidden z-50">
    ...
    <script>
        document.querySelectorAll('.variant-card').forEach(card => {
            card.addEventListener('click', function () {
                // Hapus border dari semua varian
                document.querySelectorAll('.variant-card').forEach(el => el.classList.remove('border-2', 'border-blue-500'));

                // Tambahkan border ke varian terpilih
                this.classList.add('border-2', 'border-blue-500');

                // Ambil data dari varian
                const price = this.dataset.price;
                const variant = this.dataset.variant;
                const variationId = this.dataset.variationId;

                // Tampilkan ke popup (asumsi id-nya sesuai yang Mas pakai)
                document.getElementById('popupProduct').innerText = variant;
                document.getElementById('popupPrice').innerText = 'Rp. ' + parseInt(price).toLocaleString();

                // Simpan variation_id ke hidden input (agar bisa dikirim ke backend)
    ...
    <script>
        ...
        const toggleButton = category.querySelector(".toggle-dropdown");
        ...
        toggleButton.addEventListener("click", function(event) {
            ...
            document.querySelectorAll(".payment-category").forEach(cat => {
                const catOptions = cat.querySelector(".payment-options");
                const catIcon    = cat.querySelector("i");
                if (cat !== category) {
                    catOptions.classList.add("hidden");
                    catOptions.classList.remove("flex");
...
{{-- <---DONE MENAMPILKAN USER ID-SERVERNYA-->> --}}
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        const totalPriceElement = document.getElementById("popupPrice");
        ...
        openPopupBtn.addEventListener("click", function () {
            ...
            fetch(`/get-nickname?user_id=${userID}&server_id=${serverID}`)
                ...
                .then(data => {
                    ...
                    popupProduct.textContent   = selectedProduct
                                                    .name
                                                    .replace("diamond", "")
                                                    .replace(/\B(?=(\d{3})+(?!\d))/g, ".")
                                                  + " Diamonds";
                    ...
                    // **TAMBAHAN: Tampilkan sebagai flex supaya centering bekerja**
        ...
        cancelOrderBtn.addEventListener("click", () => {
            ...
            popupContainer.classList.remove("flex");  // **TAMBAHAN**
        });

        // Confirm order (localStorage + redirect)
